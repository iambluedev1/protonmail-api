...
class ProtonMail {
  ...
  async sendEmail (options) {
    const { to, subject, body } = options
	  ...
	  await this._page.click('.compose');
	  await this._page.waitForSelector('.composer-container');
	  await this._page.select("select[id^='fromMessage']", this._config.username);
	
	  await this._page.evaluate((msg) => {
		  const $ = window.$; 
		  var body = $(".squireIframe").contents().find("body");
		  body.prepend(msg);
	  }, body);
	
	  await this._page.evaluate((to) => {
		  const $ = window.$; 
		  return $(".composer-container .awesomplete").find("input[role='combobox']").val(to)
	  }, to);
	
	  await this._page.click('.composer-container .awesomplete');
	
	  await this._page.click('input[title="Subject"]');
	  const input = await this._page.$('input[title="Subject"]');
	  await input.press('Backspace');
	  await input.type(subject);
	
	  await this._page.click('.composer-btn-send');
